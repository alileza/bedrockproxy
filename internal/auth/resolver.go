...
// It skips keys it has already resolved in this process lifetime.
...
func (r *Resolver) resolve(ctx context.Context, accessKeyID string) {
	...
	displayName := fmt.Sprintf("account:%s/%s", accountID, accessKeyID)

	_, err = r.pool.Exec(ctx, `
		UPDATE callers SET account_id = $1, display_name = $2
		WHERE access_key_id = $3 AND (account_id IS NULL OR account_id = '')
	`, accountID, displayName, accessKeyID)
	if err != nil {
